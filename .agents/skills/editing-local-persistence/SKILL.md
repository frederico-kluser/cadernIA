...
- Export helpers live in `lib/export.ts`. `downloadNote(project, format)` writes `project.content` to a `.md` or `.txt` blob, and `downloadNoteImage(node, project)` renders a DOM node (e.g. the current notebook page) to a `.png` using `html-to-image` (`lib/export.ts`).
