...
- **`.stage-wrap` é o ÚNICO scroll do app**: `html, body, #root` e `.app-root` têm `overflow: hidden` (o body rolava junto com a folha no mobile — teclado virtual, transição de `dvh` e `scrollIntoView` do textarea estouravam a viewport). Nunca reintroduzir scroll no body; conteúdo novo que excede a tela deve rolar dentro de um container próprio (`src/index.css`).
