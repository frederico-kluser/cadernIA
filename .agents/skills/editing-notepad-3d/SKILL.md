...
- The sheet lives inside `.stage-wrap` (`overflow-y: auto`). The page itself is fixed at viewport height (`height: 100%` na cadeia `.page-perspective` → `.page-stack` → `.page-current`) and does not grow with content. Content overflow scrolls inside `.page-body` (`overflow-y: auto`) (`src/index.css`).
- `.page-current` uses `position: relative`, `display: flex; flex-direction: column`, and `height: 100%` so it always covers exactly the viewport — never more, never less. `.page-body` provides internal scrolling for content overflow (`src/index.css`).
...
- The ruled-line background is applied to the mirror with default `background-attachment` (scroll), so it moves with the mirror as `.page-body` scrolls. The baseline offset `calc(var(--editor-lh) / 2 + var(--editor-font-size) * 0.334)` must be preserved (`src/index.css`).
...
- **Page height and slide/tear**: pages are fixed at viewport height (`height: 100%`), so `lastHeightRef` always captures viewport height. The `--leaving-height` / `min-height` logic is preserved (harmless no-op) and would become relevant again if pages were ever allowed to grow (`src/components/PageSheet.tsx`).
- **No internal editor scroll**: `.ghost-editor-input` uses `overflow: hidden` and its height is synchronized to `scrollHeight` after every value change. The scroll container for content overflow is `.page-body`; `.stage-wrap` handles any stage-level overflow but normally does not scroll since the page is fixed-height (`src/components/GhostEditor.tsx`).
- **Scroll só em containers internos, nunca no body**: `html, body, #root` e `.app-root` têm `overflow: hidden` (o body rolava junto com a folha no mobile — teclado virtual, transição de `dvh` e `scrollIntoView` do textarea estouravam a viewport). A página é fixa (`height: 100%`); o conteúdo excedente rola dentro de `.page-body` e o palco tem `.stage-wrap` com `overflow-y: auto` como fallback. Nunca reintroduzir scroll no body (`src/index.css`).
- **No mobile (< 640px) o alinhamento da folha é por padding, não por `left`**: `.page-header` é sticky, então o media query ajusta `padding-left: 18px` (combinando com `.ghost-editor-mirror/.ghost-editor-input/.md-preview`); ajustar `left` não tem efeito. O `padding-bottom` do editor/preview sobe para 120px no mobile para a barra flutuante de ações não cobrir o fim do texto (`src/index.css`).
- **Mobile: prevenir zoom do iOS nos inputs editáveis**: o iOS Safari dá zoom automático ao focar inputs com `font-size < 16px`, o que desloca a toolbar e a barra de ações flutuantes. O viewport deve ter `maximum-scale=1.0` (`index.html:5@63a3837`). No breakpoint mobile, `.ghost-editor-input` e `.ghost-editor-mirror` devem usar `font-size: max(var(--editor-font-size, 17px), 16px)` com `line-height` correspondente — **sempre juntos**, para não separar o cursor do texto visível (`src/index.css:808-817@63a3837`). O input de título `.page-name` também deve ter `font-size: 16px` no mobile (`src/index.css:818-820@63a3837`).
